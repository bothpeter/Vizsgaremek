        ...
        this.apiService.get('user_physique').subscribe({
            next: (response: any) => {
                if (response.status === 200 && response.UserPhysique.length > 0) {
                    const userPhysique = response.UserPhysique[0];

                    this.weight = userPhysique.weight.toString();
                    this.height = userPhysique.height.toString();
                    this.age = userPhysique.age.toString();
                    this.gender = userPhysique.gender === 'ferfi' ? 'male' : 'female';

                    this.calorieWeight = userPhysique.weight.toString();
